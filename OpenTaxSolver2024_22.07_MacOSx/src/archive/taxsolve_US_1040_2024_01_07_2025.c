...
/*-----------------------------------------------------------------*/
void capgains_qualdividends_worksheets( int status )			/* Not updated for 2024. */
{
 int j;
...
/*----------------------------------------------------------------------------------------------*/
double form6251_AlternativeMinimumTax( int itemized )						/* Not updated for 2024. */
{
...
void CapitalLossCarryOverWorksheet( char *fedlogfile, struct FedReturnData *LastYearsReturn )	/* Not updated for 2024. */
...
/*									*/
/************************************************************************/
void get_cap_gains()		/* This is Schedule-D. */			/* Not updated for 2024. */
{
...
/*--------------------------------------------------------*/
void sched_D_tax_worksheet( int status )			/* Not updated for 2024. */
{
 double ws[100];
 int k;
...
/*--------------------------------------------------------*/
void SocSec_Worksheet()							/* Not updated for 2024. */
{	/* Depends on Sched1, lines 11-25, and 1040 lines 1-8. Sets L[6]. */
 ...
 if (status == MARRIED_FILING_JOINTLY)
  ws[8] = 32000.0;      						/* Not updated for 2024. */
 else
  ws[8] = 25000.0;
 ...
 if (status == MARRIED_FILING_JOINTLY)
  ws[10] = 12000.0;      						/* Not updated for 2024. */
 else
  ws[10] = 9000.0;
...
{ /* Depends on Sched1 lines 11-25, and 1040 L[9].  */
 ...
 if (Sched1[21] != 0.0)
  { /* Student loan interest calculation pg 93. */
   ...
   ws[4] = ws[2] - ws[3];
   if (status != MARRIED_FILING_JOINTLY)
    ws[5] = 75000.0;				/* Not updated for 2024. */
   else 
    ws[5] = 155000.0;  /*MFJ*/
   if (ws[4] > ws[5])
    {
...
/* Main									*/
/*----------------------------------------------------------------------*/
int main( int argc, char *argv[] )						/* Not updated for 2024. */
{
  ...
  { /* Std. Deduction chart for People who were Born Before January 2, 1960, or Were Blind, pg 34. */
    switch (StdDedChart_NumBoxesChecked)		/* Does not handle if someone claims you or joint-spouse as dependent. */
     {				/* (Qualifying Widow/er has same amounts as MFJ, so not broken into separate variable.) */
      case 1: 
	S_STD_DEDUC   = 15700.0;					/* Not updated for 2024. */
	MFJ_STD_DEDUC = 29200.0;
